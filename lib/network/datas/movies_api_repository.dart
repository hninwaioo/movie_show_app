...
import 'package:movies_show_app/network/responses/tmdb_movies_response.dart';
import 'package:movies_show_app/network/utils/cancel_token_ref.dart';
import 'package:movies_show_app/network/utils/dio_provider.dart';
import 'package:riverpod_annotation/riverpod_annotation.dart';
part 'movies_api_repository.g.dart';
...
class MoviesRepository {
  ...
  Future<List<TMDBMovie>> popularMovies(
      {required int page, CancelToken? cancelToken}) async {
    final url = Uri(
      scheme: 'https',
      host: 'api.themoviedb.org',
...
MoviesRepository moviesRepository(MoviesRepositoryRef ref) => MoviesRepository(
      ...
      apiKey: "a92f28e11a27e8e5938a2020be68ba9c"
...
@riverpod
Future<List<TMDBMovie>> fetchPopularMovies(
    FetchMoviesRef ref, {
      required MoviesPagination pagination,
    }) async {
  final moviesRepo = ref.watch(moviesRepositoryProvider);
  ...
  if (pagination.query.isEmpty) {
    // use non-search endpoint
    return moviesRepo.popularMovies(
      page: pagination.page,
      cancelToken: cancelToken,
    );
  } else {
